...
SMTP_HOST = clean_env_value(
    os.environ.get("SMTP_HOST", "")
)

SMTP_PORT_RAW = clean_env_value(
    os.environ.get("SMTP_PORT", "587")
)

SMTP_USER = clean_env_value(
    os.environ.get("SMTP_USER", "")
)

SMTP_PASSWORD = clean_env_value(
...
def init_db():
    ...
    try:
        ...
        cur.execute("""
            ...
            idx_customer_movie_list_customer
...
def send_otp_email(
    ...
    if not BREVO_API_KEY:
        raise RuntimeError(
            ...
            "Add BREVO_API_KEY in Render Environment."
        )

    if not BREVO_FROM:
        raise RuntimeError(
            "BREVO_FROM missing. "
            "Add the verified Brevo sender email."
        )
    ...
    html_content = (
        ...
        "background:#080808;font-family:Arial,sans-serif;"
        "color:#ffffff;\">"
        "<div style=\"max-width:520px;margin:auto;"
        "background:#121212;border:1px solid #2b2b2b;"
        "border-radius:16px;padding:28px;\">"
        "<h2 style=\"margin:0 0 12px;color:#ffc400;\">"
        "Tomesh Movies"
        "</h2>"
        ...
        "background:#1d1d1d;border-radius:12px;"
        "padding:18px;text-align:center;\">"
        + otp_text
        + "</div>"
        + "<p style=\"color:#aaaaaa;margin-top:18px;\">"
        "This OTP expires in "
        + str(OTP_EXPIRY_MINUTES)
        + " minutes.</p>"
...
def request_otp():
    ...
    try:
        ...
        flash(
            ...
            "Render me BREVO_API_KEY aur BREVO_FROM check karo.",
            "error",
        )
